import { icons, getPlaceholderImage, escapeHtml } from '../utils/helpers.js';
...
function _render(container, onClose) {
  ...
  const artistName = escapeHtml(song.artists?.primary || song.artists?.singers || 'Unknown Artist');
  const albumName = escapeHtml(song.album || '');

  container.innerHTML = `
    <div class="np-header">
      <span class="np-header-title">${escapeHtml(song.album || 'Now Playing')}</span>
      <button class="np-close-btn" id="np-close">${icons.x}</button>
    </div>
    ...
    <div class="np-song-info">
      <div class="np-song-text">
        <div class="np-song-title">${escapeHtml(song.title)}</div>
